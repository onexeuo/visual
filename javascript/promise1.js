...
### Promise 
    ...
    - 그래서 비동기작업(XMLHttpRequest)이나 타이머작업(setTimeout, setInterval)을
      수행할 때 다음줄의 코드들이 먼저 수행되는 고질적 문제가 있음
    - 콜백헬(callback hell) : 코드 실행순서를 보장하기 위해서 콜백함수를 사용하게 되는데
                              순서를 보장할 코드가 늘어나면 콜백함수의 수가 늘어나서 코드의
                              복잡도가 높아지는 문제가 있다 . 이를 콜백헬 이라고 함
...
// // console.log(todos);
// document.querySelector("#result1").innerHTML = todos;


// 비동기 처리가 완료되고 다음 작업이 수행되도록 콜백처리함
// let todos;
// const get = url => {
//     const xhr = new XMLHttpRequest();
//     xhr.open("GET", url);
...
//     console.log(todos);
//     document.querySelector("#result1").innerHTML = JSON.stringify(todos);
// }
...
//             if(xhr.readyState == 4 && xhr.status == 200){
//                 resolve(JSON.parse(xhr.response));
//                 document.querySelector("#result").innerHTML = JSON.stringify(xhr.response);
//             } else {
//                 reject(`${xhr.status} ${xhr.statusText}`);
...
// .finally(rejected => console.log(rejected));
