...
// 실습 ) 배열고차함수
// 아스키코드값: 'a'.charCodeAt(0);

const exarr = [1, 'a', 'b', 2, 3, 'c', 4, 'd', 'e', 'f', 5];


// 1 ) exarr 에서 숫자들의 합
    const plus = exarr.reduce(function(acc, curr){
...
// 2 ) 숫자들은 제곱값으로, 문자들은 아스키코드값으로 변환한 배열
    const mapArr2 = exarr.map(function(ele){
...
// 3 ) 문자들 중 아스키코드값이 홀수인 것들의 배열 만들기
    const filterArr2 = exarr.filter(function(ele){
...
// 4 ) 숫자는 문자로 , 문자는 숫자(아스키 코드값)로 변환한 배열 만들기
    const mapArr3 = exarr.map(function(ele){
        if(typeof ele === 'number'){
            return String(ele);
        }else if(typeof ele === 'string'){
            return ele.charCodeAt(0);
...
// 5 ) 모든 숫자와 모든 문자들의 아스키 코드값의 합계 구하기
...
const mapArr4 = jsonArr.map(function(obj){
    if(obj.gender === 'M'){
...
const filterArr3 = jsonArr.filter(function(obj){
    if(obj.age <= 30){
        return obj;
    }
})
...
const filterArr4 = jsonArr.filter(function(obj){
...
// 5) 각 사람들의 나이의 합계를 출력
const sum3 = jsonArr.reduce(function(acc, obj){
    return acc + obj.age;
}, 0)
console.log(sum3)
...
axios(
    ...
    // 1. '.net' 이메일을 사용하는 사용자들만 추출해서 배열 생성
        const filterArr5 = dataArr.filter(function(obj){
            if(obj.email.includes('.net')){
                return obj;
            }
        }); 
        console.log(filterArr5); //1
    ...
    // 2. 회사이름에 "Romaguera"가 포함된 사용자들만 추출해서 배열 생성
        const filterArr6 = dataArr.filter(function(obj){
            if(obj.company.name.includes('Romaguera')){
                return obj;
            }
        });
        console.log(filterArr6); //2

    // 3. "Gwenborough" 도시에 사는 모든 사용자의 id의 합계를 출력
        const sum4 = dataArr.reduce(function(acc, obj){
    ...
    // 4. id가 홀수인 사용자들 중에 lat과 lng의 합이 0 이상인 사용자들의 배열
        const filterArr8 = dataArr.filter(function(obj){
            if(obj.id % 2 ==1 && (parseFloat(obj.address.geo.lat) + parseFloat(obj.address.geo.lng)) >= 0){
                return obj;
            }
        });
    ...
    //    사용자들 중에서 zipcode가 7글자 이하인 사용자들의 배열
        const filterArr7 = dataArr.filter(function(obj){
            if(obj.id % 2 ==1 
